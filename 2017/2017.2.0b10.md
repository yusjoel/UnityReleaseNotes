...
*   Editor: Editor fails to launch on attempt to restart (note however that subsequent manual launch works fine). [(929923)](https://issuetracker.unity3d.com/issues/after-trying-to-restart-unity-it-is-not-launched), [(935345)](https://unity.com/None)
...
*   XR: Data from `headPose` is incorrect on `GestureRecognizer` events. [(944108)](https://unity.com/None)
...
*   XR: \[SPS\] Occlusion mesh is visible when rendering to an RT. [(936114)](https://unity.com/None)
...
*   Collab: Fixed an issue where users could experience an editor freeze with collab enabled and 100% packet loss on network. [(920583)](https://unity.com/None)
...
*   2D: Editing 2D primitive asset no longer result error in console. [(945229)](https://unity.com/None)
...
*   Editor: Fix so that `OnHierarchyChange()` and `EditorApplication.hierarchyWindowChanged` events are not sent when when selecting gameobjects with previews and when adding gameobjects to a scene if they have the `HideInHierarchy` flag. [(946016)](https://unity.com/None)
...
*   Package Manager: Fixed Export package window showing extra folders (Packages). [(940149)](https://unity.com/None)
*   Player: Fixed leak occuring when the player didn't have all its profiling threads shut down when the profiler window was closed without turning off record. [(941997)](https://unity.com/None)
...
*   Scripting Upgrade: Fix "About" window not displaying correctly when using latest scripting runtime. [(944305)](https://unity.com/None)
*   Scripting Upgrade: Fixed Windows Standalone player failing to load Mono when built with .NET 4.6 runtime to a path with non-ascii characters in it. [(944408)](https://unity.com/None)
*   XR: Fix for Camera.main.ScreenToWorldPoint returning invalid result when app is running in MixedReality [(888928)](https://unity.com/None)
*   XR: Fixed a crash when using multiple holographic cameras on Windows Mixed Reality.
*   XR: Fixed issue with clip planes not working on Windows Mixed Reality. [(943053)](https://unity.com/None)
*   XR: Fixed Vuforia Virtual reality support on iOS. No longer fails to link during build or revert to non-vr mode. [(936670)](https://unity.com/None)
...
*   GI: Asynchronous loading of precomputed realtime GI data, by moving the GI part of file I/O into the loading thread. `Exposed DynamicGI.IsConverged` to public API so user can know when lighting is converged. Timesliced albedo-emission rendering during load. [(710503)](https://unity.com/None), [(790887)](https://issuetracker.unity3d.com/issues/scenemanager-slash-gi-application-not-responding-during-scenemanager-dot-loadsceneasync), [(815033)](https://issuetracker.unity3d.com/issues/scene-streaming-causes-frame-rate-drop)
...
*   GI: Terrain trees can now cast shadows into the baked lightmap for terrains. Terrain tree probes are now in the top of the tree canopy to make sure probes don't end up inside the tree trunk. [(685764)](https://unity.com/None)
...
*   iOS: `iOSStatusBarStyle.BlackOpaque` and `iOSStatusBarStyle.BlackTranslucent` are obsolete. Use `iOSStatusBarStyle.LightContent` instead . [(917073)](https://unity.com/None)
...
*   Multiplayer: Added a new overload to `NetworkManager.StartClient` function which allows you to specify which port the local socket should be bound to. [(903860)](https://unity.com/None)
...
*   XR: Added `PlayerSettings` values for new Async Video reprojection Settings. [(932009)](https://unity.com/None)
*   XR: Added `XRDevice.fovZoomFactor`. [(845344)](https://unity.com/None)
...
*   2D: Fixed case of text for "Edit Physics Shape" being cutoff in Sprite Editor window regardless of window size. [(916742)](https://unity.com/None)
...
*   Animation: Added an error message to SampleAnimation when attempting to sample an animation clip without an animator. [(888031)](https://unity.com/None)
*   Animation: Disabled the Add State Machine Behaviour button while new scripts need to be recompiled on a State. [(908223)](https://unity.com/None)
...
*   Audio: Audio Clip Preview: Fixed bug where changing the audio clip in the audio source selection dialog did not stop the previous playing clip. [(874592)](https://unity.com/None)
...
*   Editor: Fixed an issue where undoing creation of a `LightProbeGroup` was throwing a `MissingReferenceException`. [(657159)](https://unity.com/None)
...
*   Editor: Fixed case of TextField changing height when entering new text, and toggles not displaying fully when no text is entered. [(912001)](https://unity.com/None)
...
*   Editor: Fixed crash when closing a window if it hasn't finished loading. [(916113)](https://unity.com/None)
...
*   Editor: Fixed crash when undocking/docking the Profiler Window. [(920862)](https://unity.com/None)
...
*   Editor: Fixed issue where gizmos where not being recalculated after scene load. [(909099)](https://unity.com/None)
*   Editor: Fixed issue where removing all profilers from profiler window still displays hierarchy of CPU profiler. [(911489)](https://unity.com/None)
...
*   Editor: Fixed potential crash when calling Close() from OnGUI(). [(903828)](https://unity.com/None)
...
*   Graphics: Fixed case of grid rendering incorrectly in the Animation Preview window. [(903454)](https://unity.com/None)
*   Graphics: Fixed issue where debug GFX overlay was drawn on top of objects in Game View. [(851379)](https://unity.com/None)
...
*   Graphics: Fixed memory leak in `Texture2DArray` and `CubeMapArrayTexture`. [(930677)](https://unity.com/None), [(919162)](https://issuetracker.unity3d.com/issues/texture2darray-dot-apply-makenolongerreadable-true-doesnt-free-heap-memory)
...
*   Scripting: Fixed APIUpdater crash when checking C# scripts with deep object initialization syntax. [(902973)](https://unity.com/None)
...
*   Services: Collab: Fixed issue whereby renaming a large folder could result in work loss. [(914442)](https://unity.com/None)
*   Services: Fixed bug that would make it not possible to create a new project from the Launcher when the XBOX or PS4 support modules were installed. [(905684)](https://unity.com/None)
*   Services: Fixed crash in Unity while using Collab and doing an update, cancelling it immediately, and updating again. [(912481)](https://unity.com/None)
*   Services: Fixed issue whereby user was not prompted to save local scene edits before updating, resulting in loss of work. [(907960)](https://unity.com/None)
*   Services: Fixed multiple UI issues and bugs in the hub and support for online tutorials. [(900532)](https://issuetracker.unity3d.com/issues/new-project-cannot-be-created-by-pressing-enter-in-launcher), [(891480)](https://unity.com/None), [(893650)](https://unity.com/None), [(884626)](https://unity.com/None), [(869503)](https://unity.com/None), [(889584)](https://unity.com/None)
...
*   Timeline: Fix to not show hidden PlayableDirectors in Timeline Editor dropdown. [(900307)](https://unity.com/None)
...
*   Timeline: Fixed drag and drop of items with varying height. [(875990)](https://unity.com/None)
*   Timeline: Fixed issue where drag and drop tracks would reverse their order. [(876343)](https://unity.com/None)
*   Timeline: Fixed Timeline Editor zoom behaviour inconsistency. [(877484)](https://unity.com/None)
*   Universal Windows Platform: When simulating the HoloLens in editor, spatial mapped data will be generated in a consistent orientation. [(898305)](https://unity.com/None)
...
*   Video: Android: Fixed issue where 4.1/4.2, bigger resolution videos would not play. [(894170)](https://unity.com/None)
...
*   Video: Fixed performance issues with VideoPlayer creation/destruction. [(888745)](https://unity.com/None)
...
*   VR: The handling of a 'X' button press is now as expected:
    ...
    *   Else inject 'Esc' into the event stream so that the developer can handle as needed. [(908990)](https://unity.com/None)
...
*   XR: Fix to stop dropping out of VR when render scale is set higher than texture memory will allow, and safely reverting back to the last known good scale setting. [(916952)](https://unity.com/None)
...
*   XR: Performance and stability during in-editor remoting has been improved for the HoloLens. [(901423)](https://unity.com/None)